		...
		where TComponent : ListViewItem
	{
		...
		protected abstract class ListViewTypeRectangle : ListViewTypeBase
		{
			...
			protected override void ValidatePosition()
			{
				...
				if (!Utilities.NearlyEqual(base_position, position, Epsilon))
				{
			...
			public override void SetPosition(Vector2 newPosition, bool updateView = true)
			{
				...
				var current_position = Owner.scrollRect.content.anchoredPosition;
				var diff = (Owner.IsHorizontal() && !Utilities.NearlyEqual(current_position.x, newPosition.x, Epsilon))
						|| (!Owner.IsHorizontal() && !Utilities.NearlyEqual(current_position.y, newPosition.y, Epsilon));
			...
			{
				if ((Owner.LayoutBridge == null) || (!Owner.Virtualization))
				{

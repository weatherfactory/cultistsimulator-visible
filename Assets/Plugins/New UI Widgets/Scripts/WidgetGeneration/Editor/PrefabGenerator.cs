...
namespace UIWidgets.WidgetGeneration
{
	...
	public abstract class PrefabGenerator
	{
		...
		protected void Generate()
		{
			try
			{
				PrefabsMenu = Utilities.LoadAssetWithGUID<PrefabsMenuGenerated>(Info.PrefabsMenuGUID);
		...
		protected virtual void GenerateDataBindSupport()
		{
			...
			{
				var type = Utilities.GetType(Info.WidgetsNamespace + "." + name + Info.ShortTypeName);
				if (type != null)
				{
		...
		protected GameObject Save(GameObject go)
		{
			...
			Utilities.SetPrefabLabel(prefab);

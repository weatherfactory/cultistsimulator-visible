namespace UIWidgets
{
	...
	public class Switch : Selectable, ISubmitHandler, IPointerClickHandler, IStylable
	{
		...
		protected virtual IEnumerator AnimateSwitch(bool state, float time)
		{
			...
			var end_time = Utilities.GetTime(UnscaledTime) + time;

			while (Utilities.GetTime(UnscaledTime) <= end_time)
			{
				var distance = 1 - ((end_time - Utilities.GetTime(UnscaledTime)) / time);

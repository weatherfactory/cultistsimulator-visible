namespace UIWidgets
{
	...
	public class ScrollButtons : MonoBehaviour
	{
		...
		public void ScrolOnHold(ScrollButtonType type)
		{
			...
			AnimationStartTime = Utilities.GetTime(UnscaledTime);
		...
		public void ScrollOnClick(ScrollButtonType type)
		{
			...
			if ((AnimationStartTime + AnimationLength) < Utilities.GetTime(UnscaledTime))
			{
				return;
			}
		...
		protected virtual IEnumerator AnimationClick(Vector3 start, Vector3 end)
		{
			...
			do
			{
				animation_position = Utilities.GetTime(UnscaledTime) - AnimationStartTime;

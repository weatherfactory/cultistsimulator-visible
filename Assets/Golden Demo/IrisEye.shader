...
Shader "Custom/IrisEye"
{
	Properties
	{
		...
		Pass
		{
			...
			{
				float2 distScroll = float2(_Time.x * 0.2 * _DistSpeed, _Time.x * 0.5 * _DistSpeed);
				fixed2 dist = (tex2D(_DistTex, i.uv + distScroll).rg - 0.5) * 2;
				fixed distMask = tex2D(_DistMask, i.uv)[0];

				fixed4 col = tex2D(_MainTex, i.uv + dist * distMask * 0.025 * _DistStrength);
				fixed bg = col.a;
				...
				col = col * (1 - effect1.a * max(bg, _EffectsLayer1Foreground)) + effect1 * effect1.a * max(bg, _EffectsLayer1Foreground);
#endif

...
//
// Copyright 2013-2021 Jetro Lauha (Strobotnik Ltd)
...
// 2021-06-28, 1.8.6 - Made sure UnityWebRequests will be properly disposed to
//                     fix Unity 2021+ warnings. Removed deprecated code.
...
#               endif
                ...
                using (GUAWebRequest gwr = finalizeAndSendHit(true))
                {
                    #if GUA_USE_WEBREQUEST
...
#       if GUA_DEBUG_WARNINGS
        if (!useHTTPS)
        {
            Debug.LogWarning("GUA useHTTPS flag is false! Always using HTTPS is strongly recommended.");
        }
...
#       endif
    ...
    private IEnumerator doWWWRequestAndCheckResult(string postDataString)
    {
        ...
        using (GUAWebRequest gwr = beginWWWRequest(postDataString))
        {
            #if GUA_USE_WEBREQUEST
            yield return gwr.SendWebRequest();
            #else

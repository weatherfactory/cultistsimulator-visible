...
namespace SecretHistories.Commands.SituationCommands
{
    public class TryActivateRecipeCommand: ISituationCommand
    {
        ...
        public bool Execute(Situation situation)
        {
            ...
            var nearbyAspects = situation.GetNearbyAspects(true);

            var h = Watchman.Get<HornedAxe>();
            var aspectsInContext = h.GetAspectsInContext(aspects, nearbyAspects);
            ...
            if (_alwaysActivateRegardlessOfRequirements || recipeToActivate.RequirementsSatisfiedBy(aspectsInContext))
            {
                ...
                situation.ReactToLatestRecipePrediction(situation.GetRecipePredictionForCurrentStateAndAspects(),new Context(Context.ActionSource.SituationEffect));

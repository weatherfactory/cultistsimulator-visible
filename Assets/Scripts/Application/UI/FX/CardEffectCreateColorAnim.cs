    ...
    public class CardEffectCreateColorAnim : CardEffectCreate {
        ...
        IEnumerator DoCreateAnim() {
            ...
            while (time < durationPhase1) {
                time += Time.deltaTime;
                graphic.color = Color.Lerp(startColor, phase1Color, time / durationPhase1);
                yield return null;
            }

            graphic.color = phase1Color;
            time = 0f;
            ...
            while (time < durationPhase2) {
                time += Time.deltaTime;
                graphic.color = Color.Lerp(phase1Color, phase2Color, time / durationPhase2);
                yield return null;
            }

            graphic.color = phase2Color;

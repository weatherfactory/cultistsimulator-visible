    ...
    public class EntityTypeDataLoader
    {
        ...
        public void LoadEntityDataFromSuppliedFiles()
        {
            ...
            IOrderedEnumerable<EntityData> allEntitiesOrdered = allEntitiesInFiles
                .Where(entityData => entityData.FlushIgnoreFromEntityData(ignoredContentGroups) == false) //entities that are inside ignored content groups are excluded
                .OrderByDescending(entityData => entityData.FlushPriorityFromEntityData(_log));
        ...
        private void RegisterLocalisedDataForEmendations(List<LoadedDataFile> locFilesToProcess, HashSet<string> localizableKeys)
        {
            foreach (var locContentFile in locFilesToProcess)
            {
                ...
                foreach (var eachObject in topLevelArrayList)
                {
                    ...
                    foreach (var eachProperty in ((JObject)eachObject).Properties())
                    {
                        //ops that can change strings should be localizable; ops that change collections can contain string ops, so they should be localizable too
                        bool propertyIsStringOrCollectionOp = EntityDataImportExtensions.propertyIsStringOperation(eachProperty.Name);
                        ...
                        if (localizableKeys.Contains(eachProperty.Name) || propertyIsStringOrCollectionOp)
                        {

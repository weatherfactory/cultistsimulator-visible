namespace UIWidgets
{
		...
		where TPoint : IComparable
	{
		...
		protected bool ListIntersection(IList<TData> items, TPoint start, TPoint end, int order, TData target)
		{
			foreach (var item in items)
			{
		...
		{
			if (AllowAutoScroll && AutoScrollEnabled && Utilities.IsPointerOverScreen())
			{
				AutoScroll();
			}
		}

namespace UIWidgets
{
	...
	public class DistanceLines : MonoBehaviour
	{
		...
		/// </summary>
		/// <param name="line">Line.</param>
		protected void ShowLine(DistanceLine line)
		{
			if (line != null)
			{
		...
		/// </summary>
		/// <param name="line">Line.</param>
		protected void HideLine(DistanceLine line)
		{
			if (line != null)
			{
		...
		public virtual void UpdateLines()
		{
			...
			{
				Lines_N45_P45(angle, rect, parent_size);
			}
			else if ((angle >= -135f) && (angle <= -45f))
			{
				Lines_N135_N45(angle, rect, parent_size);
			}
			else if ((angle >= 45f) && (angle <= 135f))
			{
				Lines_P45_P135(angle, rect, parent_size);
			}
			else
			{
				Lines_P135_N135(angle, rect, parent_size);
			}
		}
		...
		protected void Lines_N45_P45(float angle, Rect target, Vector2 parentSize)
		{
		...
		/// <param name="target">Target.</param>
		/// <param name="parentSize">Parent size.</param>
		protected void Lines_N135_N45(float angle, Rect target, Vector2 parentSize)
		{
			angle += 180;
			Lines_P45_P135(angle, target, parentSize);
		}
		...
		/// <param name="target">Target.</param>
		/// <param name="parentSize">Parent size.</param>
		protected void Lines_P45_P135(float angle, Rect target, Vector2 parentSize)
		{
			var angle_rad = angle * Mathf.Deg2Rad;
			var cos = Mathf.Cos(angle_rad);
			var sin = Mathf.Sin(angle_rad);
		...
		/// <param name="target">Target.</param>
		/// <param name="parentSize">Parent size.</param>
		protected void Lines_P135_N135(float angle, Rect target, Vector2 parentSize)
		{
			if (angle > 135)
			{
				angle -= 180;
			...
			Lines_N45_P45(angle, target, parentSize);
		}

namespace UIWidgets
{
	...
	public abstract class ScrollBlockBase : UIBehaviourConditional, IBeginDragHandler, IDragHandler, IEndDragHandler, IInitializePotentialDragHandler, IScrollHandler, IStylable
	{
		/// <summary>
		/// Do nothing.
		/// </summary>
		...
		protected float ClampPadding(float padding)
		{
			...
			if (Mathf.Round(padding) >= (-size / 2f))
			{
				var d = Mathf.Round(padding) - center;
				var n = Mathf.RoundToInt(d / size);
				if (n > 0)
				{
					for (int i = 0; i < n; i++)
					{
			...
			else if (Mathf.Round(padding) < (-size / 2f))
			{
				var d = center - Mathf.Round(padding);
				var n = Mathf.RoundToInt(d / size);
				if (n > 0)
				{
					for (int i = 0; i < n; i++)
